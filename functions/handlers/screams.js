...
exports.getScream = (req,res)=>{
    ...
    .then(doc => {
        ...
        return db.collection('comments').orderBy('createdAt','asc').where('screamId','==',req.params.screamId).get();
    })
    .then(data => {
        screamData.comments = [];
        data.forEach((doc) => {
...
exports.deleteScream = (req,res)=>{
    ...
    .then(()=>{
        return res.json({message:'Scream has been deleted successfully'});
    })
    .catch(err => {
        console.error(err);
        return res.status(500).json({error:err.code});
